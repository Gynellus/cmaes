...
# Identify solvable levels
solvable_levels = original_summary[original_summary['completion_rate'] == 1]['generator'].unique()

# Filter data to only include solvable levels
print(f"Solvable levels: {solvable_levels}")
original_summary = original_summary[original_summary['generator'].isin(solvable_levels)]
print(f"Filtered original summary:\n{original_summary}")

# Load and process TOAD-GAN generated levels data
toadgan_results = pd.read_csv('toadgan_results.csv')
toadgan_results['level'] = toadgan_results['level'].map(level_dict)
toadgan_results = toadgan_results[toadgan_results['level'].isin(solvable_levels)]
toadgan_summary = summarize_data(toadgan_results, 20)
toadgan_summary = summarize_data_by_generator(toadgan_summary)
print(f"Filtered TOAD-GAN summary:\n{toadgan_summary}")

# Load and process TOAD-GAN fitness-sampled levels data
toadgan_fitness_results = pd.read_csv('toadgan_fitness_results.csv')
toadgan_fitness_results['level'] = toadgan_fitness_results['level'].map(level_dict)
toadgan_fitness_results = toadgan_fitness_results[toadgan_fitness_results['level'].isin(solvable_levels)]
...
print(f"Filtered TOAD-GAN Fitness summary:\n{toadgan_fitness_summary}")

# Load and process LVE generated levels data
lve_results = pd.read_csv('lve_toadgan_results.csv')
lve_results['level'] = lve_results['level'].map(level_dict)
lve_results = lve_results[lve_results['level'].isin(solvable_levels)]
lve_summary = summarize_data(lve_results, 20)
lve_summary = summarize_data_by_generator(lve_summary)
print(f"Filtered LVE summary:\n{lve_summary}")
...
for metric in metrics:
    ...
    if original.empty or toadgan.empty or toadgan_fitness.empty or lve.empty:
    ...
    for label, group1, group2 in comparisons:
        ...
        results.append([label, t_stat, p_value, mean_diff])
    ...
    for result, corrected_p_value in zip(results, corrected_p_values):
        result.append(corrected_p_value)
...
all_results_df.to_csv('paired_ttest_solvable_levels_results.csv', index=False)
