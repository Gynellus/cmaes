...
def generate_sample(generators, noise_maps, reals, noise_amplitudes, num_layer, token_list, in_states=None, scale_v=1.0,
    ...
    for G, Z_opt, noise_amp in zip(generators, noise_maps, noise_amplitudes):
        ...
        # Ensure Z_opt has a batch dimension
        # print(Z_opt.shape)
        if Z_opt.ndim == 3:
            Z_opt = Z_opt.unsqueeze(0)  # Add batch dimension
        elif Z_opt.ndim != 4:
            raise ValueError(f"Expected Z_opt to have 4 dimensions after adjustment, got {Z_opt.ndim}")
